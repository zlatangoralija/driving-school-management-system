...
export const _navItem = (data, index, active_page = []) => {
  ...
  const [expanded, setExpanded] = React.useState(
    _inArray(data.name, active_page) ? true : false
  );
  ...
  return (
    <li
      key={index}
      className={`${_inArray(data.name, active_page) ? "active" : null} ${
        data.type ? "li-" + data.type : null
      }`}
      ref={nodeDD}
    ...
    >
      {data.nested ? (
        <>
          ...
          {data.badge ? (
            <div dangerouslySetInnerHTML={{ __html: data.badge }} />
          ) : null}
          <button type="button" onClick={() => setExpanded(!expanded)}>
            <span className={expanded ? "icon-up" : "icon-down"} />
          </button>
...
export const scrollToTime = function () {
