...
export default function Book(props) {
  ...
  const initPendingBooking = async (data) => {
    ...
    if (booking_id) {
      return await axios
        .put(
          route("students.update-book-and-pay", { booking: booking_id }),
          finalData
        )
        .then((res) => {
          return {
      ...
      return await axios
        .post(route("students.store-book-and-pay"), finalData)
        .then((res) => {
          return {
            status: true,
            url: res.url,
  ...
  const book = async (data) => {
    try {
      ...
      if (booking_id) {
        router.patch(
          route("students.bookings.update", { booking: booking_id }),
          finalData,
          {
            onError: (errors) => {
              console.log(errors);
  ...
  return (
    <>
      ...
      <div className="flex justify-center">
        <button
          type="button"
          className="button button-blue button-big my-8"
        ...
        footer={
          <div className="w-full flex justify-between items-center">
            ...
            <button
              type="button"
              onClick={() =>
                props.booking?.id && props.booking?.payment_status
                  ? book(startDate)
