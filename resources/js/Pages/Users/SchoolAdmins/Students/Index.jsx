...
export default function Index(props) {
    ...
    const createActionItems = (row) => {
        return [
            {
                label: "View",
                href: route("school-administrators.students.show", {student: row.id}),
            },
            {
                label: "Edit",
                href: route("school-administrators.students.edit", {student: row.id}),
            },
            {
                label: "Delete",
                action: () => {
    ...
    const columns = [
        ...
        {
            name: "Account status",
            selector: (row) => <><div className={`button-pill button-${row.status ==1 ? 'green' : 'red'}`}>{row.status_label}</div></>,
            sortable: true,
            sortField: "status",
        },
        {
            name: "Date created",
            selector: (row) => (
                <>{timezoneDate(row.start).format("DD/MM/YYYY HH:mm")}</>
            ),
            sortable: true,
    ...
    function deleteStudent(id) {
        router.delete(
            route("school-administrators.students.destroy", {student: id}),
            {
                onSuccess: (page) => {
                },
                onError: (errors) => {
    ...
    return (
        <>
            ...
            <div className="flex justify-end mb-3">
                <Link
                    href={route("school-administrators.students.create")}
