...
export default function Index(props) {
                ...
                return(
                    <>
                        <div className="flex justify-between gap-3">
                            <Link href={route('school-administrators.students.show', {student: row.id})} className="link">View</Link>
                            <Link href={route('school-administrators.students.edit', {student: row.id})} className="link">Edit</Link>
    ...
    function deleteStudent(id){
        router.delete(route('school-administrators.students.destroy', {student: id}), {
            onSuccess: (page) => {},
            onError: (errors) => {},
            preserveState: false,
    ...
    return (
        <>
            ...
            <div className="flex justify-end mb-3">
                <Link href={route('school-administrators.students.create')} className="text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 sm:mr-2 lg:mr-0 focus:outline-none">Create new</Link>
            </div>

            <DataTableComponent
                columns={columns}
                path={route('school-administrators.get-school-students')}
                search={search}
                object={"students"}
                pagination={true}
                filters={filters}
