...
export default function CreateForm(props) {
  ...
  const submit = async () => {
    try {
      ...
      if (props.administrator) {
        router.put(
          route("school-administrators.administrators.update", {
        ...
        router.post(
          route("school-administrators.administrators.store"),
          finalData,
          {
            onError: (errors) => {
              formRef.current.setErrors(errors);
  ...
  return (
    <>
      <Head
        title={
          props.administrator ? "Edit administrator" : "Create administrator"
        }
      />
      ...
      <div className="grid grid-cols-0 md:grid-cols-0 gap-6">
        <Form ref={formRef} onSubmit={submit} className="card p-5 mb-3">
          ...
          <SelectDefault
            ...
            defaultValue={
              props.statuses && props.administrator
                ? props.statuses.find(
                    (x) => x.value === props.administrator.status
                  )
                : props.statuses[0]
            }
