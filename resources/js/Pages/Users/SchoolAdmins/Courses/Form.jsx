  ...
  const submit = async () => {
    try {
      ...
      const schema = Yup.object().shape({
        ...
        number_of_lessons: Yup.string()
          .matches(/^[0-9]+$/, "Number of lessons has to be a number.")
          .required("Please input number of lessons for this course."),
      ...
      let finalData = {
        ...formData,
        payment_option:
          formData.payment_option.length > 0
            ? formData.payment_option[0].value
            : null,
        instructor_id:
          formData.instructor_id.length > 0
            ? formData.instructor_id[0].value
            : null,
        price: formData.price
          ? parseFloat(formData.price.replace("$", "").replace(/,/g, ""))
          : null,
      };

      if (props.course) {
        router.put(
          route("school-administrators.courses.update", {
            course: props.course,
          }),
          finalData,
          {
    ...
    <>
      ...
      <div className="mx-auto mt-6 mb-10" ref={wrapperRef}>
        <h1 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">
          {props.course ? "Edit course" : "Create course"}
        </h1>
      ...
      <div className="grid grid-cols-0 md:grid-cols-0 gap-6">
        <Form ref={formRef} onSubmit={submit} className="card p-5 mb-3">
          ...
          <SelectDefault
            options={Object.entries(props.instructors).map(
              ([value, label]) => ({ value, label })
            )}
            defaultValue={
              props.instructors && props.course
                ? Object.entries(props.instructors)
                    .map(([value, label]) => ({ value, label }))
                    .find((x) => x.value == props.course.instructor_id)
                : ""
            }
            label="Instructor*"
            name="instructor_id"
          ...
          <SelectDefault
            ...
            defaultValue={
              props.payment_options && props.course
                ? props.payment_options.find(
                    (x) => x.value === props.course.payment_option
                  )
                : props.payment_options[0]
            }
