  ...
  const submit = async () => {
    try {
      ...
      let finalData = {
        ...formData,
        timezone:
          formData.timezone.length > 0 ? formData.timezone[0].value : null,
      };
      ...
      router.put(
        route("school-administrators.update-account-settings"),
        finalData,
        {
          onError: (errors) => {
      ...
      <div className="grid grid-cols-0 md:grid-cols-0 gap-6">
        <Form ref={formRef} onSubmit={submit} className="card p-5 mb-3">
          ...
          <SelectDefault
            name="timezone"
            label="Timezone*"
            options={Object.entries(props.timezones).map(([value, label]) => ({
              value,
              label,
            }))}
            defaultValue={
              props.timezones && props.current_timezone
                ? Object.entries(props.timezones)
                    .map(([value, label]) => ({ value, label }))
                    .find((x) => x.value == props.current_timezone)
                : ""
            }
          />
