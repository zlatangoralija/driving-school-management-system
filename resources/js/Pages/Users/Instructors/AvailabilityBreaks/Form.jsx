...
export default function CreateForm(props) {
    ...
    const [ endDate, setEndDate ] = React.useState(props.break?.end_time ? new Date(props.break.end_time) : null)
    ...
    const filterPassedTime = (time) => {
        ...
        if(props.breaks && props.breaks.length){
            const excluded = props.breaks.map(time => setHours(setMinutes(new Date(time.date), time.minutes), time.hours))
    ...
    const submit = async(data) => {
        ...
        try{
            ...
            if(props.break){
                router.put(route('instructors.availability-breaks.update', {availability_break:props.break}), finalData, {
                    onError: (errors) => {
                        formRef.current.setErrors(errors);
                    }
                })
            }else{
                router.post(route('instructors.availability-breaks.store'), finalData, {
                    onError: (errors) => {
                        formRef.current.setErrors(errors);
                    }
                })
    ...
    return (
        <>
            <Head title={props.break ? 'Edit availability break' : 'Create new availability break'} />

            <div className="mx-auto mt-6 mb-10" ref={wrapperRef}>
                <h1 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{props.break ? 'Edit availability break' : 'Create new availability break'}</h1>
